...
public sealed class SeriesProjectionTests : IDisposable
{
    ...
    /// <summary>
    /// An entry of a kind that is neither a film, an episode nor a show is no row at
    /// all. Nothing the endpoints accept can put one on a list, so what this covers is a
    /// document written by something that is not this plugin - and the answer it gets is
    /// a missing row rather than a projection pass that stops.
    /// </summary>
    [Fact]
    public void AnEntryOfAKindTheListDoesNotHoldIsNoRow()
    {
        var store = AStoreHolding((ASong, WatchlistItemKind.Other), (AFilm, WatchlistItemKind.Movie));
