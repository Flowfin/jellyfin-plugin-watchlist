...
| `nightly-betas`, `publish-beta`, `publish-jf12-beta`, `publish-jf12-stable` | One release has been published here, `0.1.0.0-stable`, by the stable route in `publish.yaml`, and there is no beta channel to publish to: the hub's catalogue carries finished releases only, which is entry 5 of its own #1. The second line's route is #4 behind a stable 12.0 server release, and the manifest every install reads is #74. |
