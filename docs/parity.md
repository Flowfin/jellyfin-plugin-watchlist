...
| `Report any workflow that concluded non-success on the default branch` | `Report a publish that shipped nothing`, declared and never triggered | The check the other board's `publish-failure-alert` reports under. Landed on #78, in `.github/workflows/publish-outcome-alert.yaml`. It watches from outside the publish route on `workflow_run`, because a step at the end of a job cannot report on a job that stopped before reaching it, and that is the case it exists for. It reads the release for the tag the run published and names every way the run left something nobody can install: no release, a draft, no archive, a second `.md5`, a missing checksum, missing packaging metadata, a missing inventory, a missing attestation bundle. The asset set it expects is read off `.github/workflows/publish.yaml` rather than decided there. It raises one issue per run, keyed on the run id, so closing the issue for one run suppresses nothing for the next. It has never triggered: the publish route runs on a pushed tag, no tag has been pushed, and the run count of that workflow path is 0. Two things are not covered and #78 stays open for them. The alert does not cover the scheduled manifest check, which is #89 and does not exist, so there is no second workflow to name in its trigger. And the raising itself is unproven: the judgement has probes for every shape it alerts on and every shape it stays silent on, running in the job ahead of the reading, but the API call that opens the issue has never been watched, because a deliberately broken publish costs a version number. |
